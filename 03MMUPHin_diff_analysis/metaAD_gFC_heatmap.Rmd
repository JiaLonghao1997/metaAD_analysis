...
for (taxon_type in taxon_types){
    ...
    for (stage in stages){
        for (feat_type in feat_types){
            ...
            diff_count = nrow(meta_results[meta_results$pval<0.01, ])
            diff_species = row.names(meta_results[meta_results$pval<0.01, ])
...
if(taxon_type %in% c("KOs")){
    ...
    title = "Differential gut metabolic modules (GMMs)"
    ...
    #merge_reporter_filter = merge_reporter[(rowSums(reporter_df>1.7)>=1) | (rowSums(reporter_df < -1.7)>=1),  ]
    title = "Differential gut–brain modules (GBMs)"
...
imglist = list()
outdir = "D:\\Zhaolab2020\\gut-brain-axis\\metaAD\\01MLmodels\\multikingdom\\03MMUPHin_diff"
for(taxon_type in taxon_types){
    #taxon_type = "pathways"
    diff_KOs_dir = file.path("D:\\Zhaolab2020\\gut-brain-axis\\metaAD\\01MLmodels\\multikingdom\\03MMUPHin_diff", taxon_type)
    MMUPHin_stat_file = file.path(diff_KOs_dir, "01up_down_stage.csv")
    MMUPHin_stat_df = read.csv(MMUPHin_stat_file, header=1)
    MMUPHin_stat_df = MMUPHin_stat_df[MMUPHin_stat_df$count>0, ]
    ...
    img <- ggplot(data = MMUPHin_stat_df, mapping = aes(x = stage, y = count, label=count)) + 
        geom_bar(aes(fill=change), stat = 'identity', position = "stack") +
    geom_text(size = 4, position = position_stack(vjust = 0.5)) + 
        scale_fill_manual(values=colors) + 
        labs(title=paste0("Numbers of differential ", taxon_type)) + 
        scale_y_continuous(breaks=seq(0,end,step), labels=seq(0,end,step), limits=c(0,end)) +
        theme_bw() + 
        theme(
...
pdf(file.path(outdir, "diff_KOs_GMMs_GBMs_pathways_stat.pdf"), width=12, height=8)
img = ggarrange(imglist$KOs, imglist$GMMs, imglist$GBMs, imglist$pathways, nrow=2, ncol=2, align="hv", labels=letters[1:4])
print(img)
dev.off()
```
