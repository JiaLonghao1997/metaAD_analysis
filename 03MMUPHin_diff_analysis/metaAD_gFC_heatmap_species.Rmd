...
for (taxon_type in taxon_types){
    ...
    for (stage in stages){
        for (feat_type in feat_types){
            ...
            diff_count = nrow(meta_results[meta_results$pval<0.001, ])
            diff_species = row.names(meta_results[meta_results$pval<0.001, ])
...
if(taxon_type %in% c("KOs")){
    ...
    title = "Differential species (P<0.05 in at least two stages)"
    width=8
    height=8
    merge_gFC = gFC
}
...
pdf(file.path(diff_KOs_dir, "diff_KOs_stat.pdf"), width=6, height=3)
print(img)
dev.off()
```
