...
def plot_auroc_curve(train_study, stage, model, fs_method, auroc_curve_dic, outdir):
    ...
    plt.title(label="NC vs {} (cross validation)".format(stage), fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "auroc_{}_{}_{}_{}.png".format(
        train_study, stage, model, fs_method)), dpi=300)
    plt.savefig(os.path.join(outdir,"auroc_{}_{}_{}_{}.pdf".format(
        train_study, stage, model, fs_method)), dpi=300)
...
suffix = '+ConQuR'
#suffix = ''
taxons = ['species'+suffix, 'KOs'+suffix, 'GMMs'+suffix, 'GBMs'+suffix, 'pathways'+suffix]
