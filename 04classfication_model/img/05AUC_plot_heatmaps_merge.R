...
#fs_methods = c('MMUPHin+RFECV', 'MMUPHin_stage+RFECV', 'RandomForest+RFECV','RandomForest+RFECV_X2','RandomForest+RFECV_X4', 'all+Boruta', 'MMUPHin+Boruta')
fs_methods = c("merge")
imglist = list()

for (fs_method in fs_methods){
    for(metric in c('AUC', 'n_features')){
        ...
        if (file.exists(infile)){
            ...
            img <- Heatmap(data, name = metric, col = col_fun, cluster_rows = FALSE, cluster_columns = FALSE, 
                    rect_gp = gpar(col = "white", lwd = 1),
                    cell_fun = function(j, i, x, y, width, height, fill) {
                        grid.text(sprintf(text_format, data[i, j]), x, y, gp = gpar(fontsize = 14))},
                    column_split = c('A', 'A', 'A', 'A', 'B'),
                    ...
                    row_names_side = "left", column_names_side = "bottom",
                    column_title_side = "bottom", 
                    column_title = "Stages", row_title = "Features",
                    column_title_gp = gpar(fontsize=14),
                    row_title_gp = gpar(fontsize=14),
                    row_dend_gp = gpar(fontsize=14), 
                    column_dend_gp = gpar(fontsize=14),
...
# draw(imglist[[1]]+imglist[[2]]+imglist[[3]]+imglist[[4]]+imglist[[5]]+imglist[[6]], padding = unit(c(2, 2, 2, 2), "mm"), heatmap_legend_side="top", show_heatmap_legend=TRUE)
# dev.off()
