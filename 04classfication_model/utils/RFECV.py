...
def RFECV_clf(X, y, feature_list, fs_method, outdir, stage, study, feat_type, model, repeats, fold):
    ...
    auroc_mean = clf.cv_results_['mean_test_score'][n_features]
    auroc_std = clf.cv_results_['std_test_score'][n_features]

    #selected_features = (X[:, clf.get_support()])

    print(f"Optimal number of features: {n_features}")   ##最佳特征数目是多少。
    print("REF({}-{}-{}-{}-{},{} features, AUC={:.2f})".format(study, model, feat_type, fs_method, stage, n_features, auroc_mean))
    plot_REFCV(clf, min_features_to_select, outdir, study, model, feat_type, fs_method, stage, n_features, auroc_mean)

    #results.append([stage, study, feat_type, model,n_features_in, n_features, selected_features, auroc])
    result = [stage, study, feat_type, fs_method, model, auroc_mean, auroc_std, n_features_in, n_features, selected_features]
    return(result)
    #return(stage, study, feat_type, model,n_features_in,n_features,selected_features,auroc)


def testwork(workdir):
    ...
    repeats = 3
    fold = 5
    result = RFECV_clf(X_train, y_train, feature_list, fs_method, outdir, stage, study, feat_type, model, repeats, fold)
    print(result)


def main():
    workdir = "/share/home1/jialh/brain/01meta/multikingdom/"
