...
for root,dirs,files in os.walk(DATA_DIR):
    for file in files:
        if file.endswith(EXTENSION):
            ...
            infile1 = join(DATA_DIR, sample, sample + '_' + READ_SUFFIX[0] + EXTENSION)
            infile2 = join(DATA_DIR, sample, sample + '_' + READ_SUFFIX[1] + EXTENSION)
            #print("infile1: {}, infile2: {}".format(infile1, infile2))
            if (not sample.endswith(EXTENSION)) and os.path.exists(infile1) and os.path.exists(infile2):
...
rule pre_fastqc:
    input:  
        fwd = join(DATA_DIR, "{sample}", "{sample}_" + READ_SUFFIX[0] + EXTENSION),
        rev = join(DATA_DIR, "{sample}", "{sample}_" + READ_SUFFIX[1] + EXTENSION)
    output:
    ...
    shell: """
        ...
        /home1/jialh/human_virome/tools/FastQC/fastqc {input} --outdir {params.outdir} --threads {threads}
    """
...
rule pre_multiqc:
    ...
    shell: """
        ##/home1/jialh/tools/anaconda3/bin/multiqc
        /home1/jialh/tools/anaconda3/bin/multiqc --force {params.indir} -o {params.outdir}
    """
...
rule deduplicate:
    ...
    shell: """
        ...
        /home1/jialh/metaHiC/tools/biotools/HTStream_v1.3.0/hts_SuperDeduper -1 {input.fwd} -2 {input.rev} -f {wildcards.sample} -F
...
rule trim_galore:
    ...
    shell: """
        mkdir -p {params.outdir}
        /home1/jialh/metaHiC/tools/biotools/TrimGalore-0.6.7/trim_galore \
        --path_to_cutadapt /home1/jialh/anaconda3/envs/cutadapt/bin/cutadapt \
        ...
        zcat -f {params.orp_fwd} {params.orp_rev} | /home1/jialh/tools/miniconda3/bin/pigz -b 32 -p {threads} > {output.orp}
...
rule rm_host_reads:
    ...
    shell: """
        ...
        # run on paired reads
        /home1/jialh/tools/miniconda3/bin/bwa mem -t {threads} {params.bwa_index_base} {input.fwd} {input.rev} | \
            /home1/jialh/tools/anaconda3/bin/samtools fastq -@ {threads} -t -T BX -f 4 -1 {output.unmapped_1} -2 {output.unmapped_2} -s {params.singelton_temp_1} -
        # run on unpaired reads
        /home1/jialh/tools/miniconda3/bin/bwa mem -t {threads} {params.bwa_index_base} {input.orp} | \
            /home1/jialh/tools/anaconda3/bin/samtools fastq -@ {threads} -t -T BX -f 4 - > {params.singelton_temp_2}
        # combine singletons 
        zcat -f {params.singelton_temp_1} {params.singelton_temp_2} | /home1/jialh/tools/miniconda3/bin/pigz -p {threads} > {output.unmapped_singletons}
...
rule post_fastqc:
    ...
    shell: """
        ...
        if [ -z $(gzip -cd {input} | head -c1) ]; then
            ...
            /home1/jialh/human_virome/tools/FastQC/fastqc {input} -f fastq --outdir {params.outdir} -t {threads}
        fi
    """
...
rule readcounts:
    input:
        raw = expand(join(DATA_DIR, "{sample}", "{sample}_" + READ_SUFFIX[0] + EXTENSION), sample=SAMPLE_PREFIX),
    ...
    run:
        ...
        with open(outfile, 'w') as outf:
            ...
            for sample in SAMPLE_PREFIX:
                print("readcounts: {}".format(sample))
                raw_file = join(DATA_DIR, "{sample}", "{sample}_" + READ_SUFFIX[0] + EXTENSION)
