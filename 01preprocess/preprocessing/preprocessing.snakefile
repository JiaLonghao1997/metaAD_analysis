...
rule trim_galore:
    ...
    shell: """
        mkdir -p {params.outdir}
        trim_galore \
        --path_to_cutadapt /home1/jialh/tools/miniconda3/bin/cutadapt \
...
rule rm_host_reads:
    ...
    shell: """
        ...
        /home1/jialh/tools/miniconda3/bin/bwa mem -t {threads} {params.bwa_index_base} {input.fwd} {input.rev} | \
            /home1/jialh/tools/samtools-1.9/samtools fastq -@ {threads} -t -T BX -f 4 -1 {output.unmapped_1} -2 {output.unmapped_2} -s {params.singelton_temp_1} -
        # run on unpaired reads
        /home1/jialh/tools/miniconda3/bin/bwa mem -t {threads} {params.bwa_index_base} {input.orp} | \
            /home1/jialh/tools/samtools-1.9/samtools fastq -@ {threads} -t -T BX -f 4 - > {params.singelton_temp_2}
        # combine singletons 
    ...
    run:
        ...
        with open(outfile, 'w') as outf:
            ...
            for sample in SAMPLE_PREFIX:
                ...
                raw_file = join(DATA_DIR, sample, sample + READ_SUFFIX[0] + EXTENSION)
