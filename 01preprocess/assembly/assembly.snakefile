...
rule megahit:
    ...
    shell: """
        rm -rf {params.outdir}
        megahit --presets meta-sensitive {params.reads_command} -o {params.outdir} -t {threads} --out-prefix {wildcards.sample}
    """
...
rule quast_megahit:
    ...
    shell: """
        /home1/jialh/anaconda3/envs/quast/bin/quast.py -o {params.outdir} {input} --fast
        """
...
rule combine_megahit_quast_reports_R:
    ...
    script: "python scripts/combine_quast_reports.py"
...
rule spades:
    ...
    resources:
        time = lambda wildcards, attempt: 24 * attempt,
        mem = lambda wildcards, attempt: 400 * attempt
    threads: 16
