#!/usr/bin/env python
#https://github.com/NBISweden/nbis-meta/blob/main/workflow/scripts/eggnog-parser.py
#说明:本代码主要包括download, parse, quantify和merge共4个函数，位于第310行至330行。
...
# Download functions
####################
##ko00002相关说明位于: https://www.genome.jp/kegg-bin/show_brite?ko00002.keg
def get_kegg_module_hierarchy(s):
    ...
    for d1 in s['children']:
        ...
        for d2 in d1['children']:
            c2 = d2['name']  ##Carbohydrate metabolism之类的。
            for d3 in d2['children']:
                c3 = d3['name']   ##Central carbohydrate metabolism
                for module in d3['children']:
                    module_name = module['name']  ##M00001  Glycolysis (Embden-Meyerhof pathway), glucose => pyruvate [PATH:map00010 map01200 map01100]
                    #kos = module['children']
                    hier[module_name] = {"Module_category1": c1,
                                         "Module_category2": c2,
                                         "Module_category3": c3}
                    #for ko in kos:
                    #    ko_name = ko['name']
...
##相关文件参考：https://www.genome.jp/kegg-bin/show_brite?ko00001.keg
def get_kegg_ortholog_hierarchy(s):
    hier = {}
    # First level is 'ko00001'
    for d1 in s['children']:
        c1 = d1['name']  ##09100 Metabolism
        for d2 in d1['children']:
            c2 = d2['name']  ##09101 Carbohydrate metabolism
            for d3 in d2['children']:
                c3 = d3['name']  ##00010 Glycolysis / Gluconeogenesis [PATH:ko00010]
                if not "children" in d3.keys():
                    continue
                for ko in d3['children']:
                    ko_name = ko['name'].split("\t")[0]   ##K00844  HK; hexokinase [EC:2.7.1.1]
                    ko_id = ko_name.split(" ")[0]
                    if "[EC:" in ko_name:
                        enzymes = ko_name.split("[")[-1].split("]")[0].lstrip("EC:").split(" ")  ###2.7.1.1
                    else:
                        enzymes = []
...
def get_kegg_ortholog_info(outdir, tmpdir="$TMPDIR"):
    ...
    with open(tmp_out) as fh:
        s = json.load(fh)
    hier = get_kegg_ortholog_hierarchy(s)  ##定义位于第38行。
    pathways = {}
    ec2path = {}
    ...
    with open(ko_out, 'w') as fh_kos, open(ko2path_out, 'w') as fh_ko2path, open(ko2ec_out, 'w') as fh_ko2ec:
        ...
        for ko_id, l in hier.items():  ##开始遍历字典。
            for i, d in enumerate(l):  ##遍历内部字典，字典的键-值为{"KO_category1": c1, "KO_category2": c2, "pathway": c3, "name": ko_name, "enzymes": enzymes}
                if i == 0:
                    fh_kos.write("{}\t{}\n".format(ko_id, d["name"])) ##记录KOs.
                    for enzyme in d["enzymes"]:
                        fh_ko2ec.write("{}\t{}\n".format(ko_id, enzyme))  ##KOs中的酶。
                fh_ko2path.write("{}\t{}\n".format(ko_id, "map{}".format(d["pathway"].split(" ")[0])))  ##记录KO所属pathway.
                pathways[d["pathway"]] = {"Pathway_category1": d["KO_category1"],
                                          "Pathway_category2": d["KO_category2"]}  ##记录pathways所属类别。
                for enzyme in d["enzymes"]:
                    try:
...
def get_kegg_module_info(outdir, tmpdir="$TMPDIR"):
    ...
    request.urlretrieve(url, tmp_out)  ##将URL表示的网络对象复制到本地文件。
    modules_out = "{}/kegg_modules.tsv".format(outdir)
    with open(tmp_out) as fh:
        s = json.load(fh)  ##json.load()是用来读取文件的，即，将文件打开然后就可以直接读取。
    hier = get_kegg_module_hierarchy(s)  ##定义位于第16行。
    ...
    with open(modules_out, 'w') as fh_modules:
        ...
        for module_name, d in hier.items():  ##本质上是字典的遍历。
...
def parse_ko_annotations(annotations, dldir, outdir, map_go=False):
    ...
    annot = pd.read_csv(annotations, header=None, usecols=[0, 2, 3, 7, 9, 11, 12, 13, 17, 18],
                        names=["orf", "evalue", "score", "desc", "GO", "ko", "pathway", "module", "tc", "cazy"],
                        sep="\t")  ##原始文件中对应的列为:#query, evalue, score, COG_category,
    ##========================================
...
def parse(args):
    for f in ["kegg_ec2pathways.tsv", "kegg_ko2ec.tsv", "kegg_ko2pathways.tsv",
        ...
        if not os.path.exists("{}/{}".format(args.dldir, f)):
            download(args)
            break
    parse_ko_annotations(args.annotations, args.dldir, args.outdir, args.map_go)  ##相关定义位于第175行。
