...
study.shapes <- unlist(parameters$plotting$study.shapes)
...
outdir = file.path(workdir, "02diversity_KOs")
...
print(summary(metadata_features$host_removed_count))
print(metadata_features[metadata_features$host_removed_count>50, 1:15])
...
print(summary(metadata_features_filter$host_removed_count))
###
...
(raremax <- quantile(rowSums(features_filter), c(0.1))[1])
...
if(file.exists(features_rar_file)){
    ...
    features_rar = rrarefy(round(features_filter), sample=raremax)
...
###<--------------------------------------------->###
library(vegan)
##The abundance-based estimates in estimateR use counts (numbers of individuals) of species in a single site. 
##If called for a matrix or data frame, the function will give separate estimates for each site. 
estimateR = t(estimateR(feat.rar))[,c(1,2,4)]
colnames(estimateR) = c("richness", "chao1", "ACE")

shannon = vegan::diversity(feat.rar, index = "shannon")
simpson = vegan::diversity(feat.rar, index = "simpson")
invsimpson = vegan::diversity(feat.rar, index = "invsimpson")
...
###
meta_alpha_div = merge(meta, alpha_div, by.x="row.names", by.y="row.names")
meta_alpha_div_melt = reshape2::melt(meta_alpha_div, measure.var=c("richness", "chao1", "ACE", "shannon", "simpson", "invsimpson"), 
                                     variable.name="type", value.name = "diversity")
print(unique(meta_alpha_div_melt$Group))
meta_alpha_div_melt$Group <- factor(meta_alpha_div_melt$Group, levels=c("NC", "SCS", "SCD", "MCI", "AD"))


# anova
plot_alpha_diversity <- function(meta_alpha_div_melt, stage, type){
    #type = "Simpson"
    ...
    #rm(aov_df)
    aov_df <- summary(aov(rank~Group+Batch, data=meta_alpha_div_melt %>% 
                              filter(type==type) %>% 
                              mutate(rank=rank(diversity))))
    print(aov_df)
    Pvalue <- signif(aov_df[[1]][["Pr(>F)"]][1], 2)
    meta_alpha_div_melt_filter <- meta_alpha_div_melt[meta_alpha_div_melt$type==type,]
    
    print(dim(meta_alpha_div_melt_filter))
    print(unique(meta_alpha_div_melt_filter$type))
    
    library(stringr)
    max_diversity <- max(meta_alpha_div_melt_filter$diversity)
    g <- meta_alpha_div_melt_filter %>% 
      ggplot(aes(x=Group, fill=Group, y=diversity)) +
        geom_boxplot() +
        theme_bw() + 
        labs(x="Groups", y=paste0(str_to_title(type), " diversity"), 
             title=paste0(str_to_title(type), " diversity"),
             subtitle=paste0("ANOVA P-value = ", Pvalue)) +
        theme(
        ...
        stat_compare_means(label = "p.format", method = "wilcox.test", 
                           comparisons = my_comparisons, size=2, data=meta_alpha_div_melt_filter)
...
richness_img <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="richness")
chao1_img <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="chao1")
ACE_img  <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="ACE")

Shannon_img <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="shannon")
Simpson_img <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="simpson")
invsimpson_img  <- plot_alpha_diversity(meta_alpha_div_melt, stage, type="invsimpson")

print(outdir)
pdf(file.path(outdir, paste0('alpha_diversity_', feat_type, '.pdf')), width = 12, height = 8)
merge_img = plot_grid(richness_img, chao1_img, ACE_img, 
                      Shannon_img, Simpson_img, invsimpson_img, 
                      ncol =3, nrow=2, align="hv", labels=letters[1:6])
print(merge_img)
dev.off()
...
##通过置换多因素方差分析，检查study和group分别在beta多样性中p-value差异。
...
study.pvalue = signif(results$`Pr(>F)`[1], 2)
...
aixs1.study.pvalue <- signif(kruskal.test(Axis1 ~ Batch, data = df.plot)$p.value, 2)
aixs2.group.pvalue <- signif(kruskal.test(Axis2 ~ Group, data = df.plot)$p.value, 2)
aixs2.study.pvalue <- signif(kruskal.test(Axis2 ~ Batch, data = df.plot)$p.value, 2)
...
g.main <- df.plot %>% 
  ...
  scale_shape_manual(values=study.shapes) + 
  scale_x_continuous(position='top') +
  xlab(axis.1.title) + ylab(axis.2.title) +
  annotate("text", x = min(df.plot$Axis1), y = min(df.plot$Axis2), hjust = 0.1, vjust = 0, 
           label= paste0("Batch: ", 'P', "=", study.pvalue, 
...
g.s.2 <- df.plot %>% 
  ...
  geom_boxplot() + 
  scale_fill_manual(values=study.cols, guide="none") +
  xlab(paste0("Batch\n", 'P', "=",  aixs2.study.pvalue)) +
...
study.shape.legend <- as_ggplot(get_legend(g.main))
study.color.legend <- as_ggplot(get_legend(g.s.1))
group.color.legend <- as_ggplot(get_legend(g.g.1))
merge_legend1 <- plot_grid(study.color.legend, study.shape.legend, nrow=2, rel_heights = c(0.2, 0.3))
